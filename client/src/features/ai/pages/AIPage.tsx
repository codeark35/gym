...
function renderAiMarkdown(text: string) {
  ...
  const formatted = escaped
    .replace(
      /^###\s+(.*)$/gm,
      '<div style="font-size:0.95rem; font-weight:700; margin:0.5rem 0 0.25rem;">$1</div>',
    )
    .replace(
      /^##\s+(.*)$/gm,
      '<div style="font-size:1rem; font-weight:700; margin:0.6rem 0 0.3rem;">$1</div>',
    )
    .replace(
      /^#\s+(.*)$/gm,
      '<div style="font-size:1.1rem; font-weight:700; margin:0.7rem 0 0.35rem;">$1</div>',
    )
...
function chatReducer(
  state: { conversations: Conversation[]; activeId: string | null },
  action: Action,
) {
  switch (action.type) {
    case "NEW_CHAT": {
...
export default function AIPage() {
      ...
      { message },
      {
        onSuccess: (data) => {
          dispatch({
            type: "ADD_MESSAGE",
            id: activeConv.id!,
            message: { role: "ai", content: data as string },
          });
        },
        onError: () => {
          dispatch({
  ...
  const handleQuickAnalysis = (type: AnalysisType) => {
    const msg =
      ANALYSIS_OPTIONS.find((o) => o.type === type)?.label ?? "Análisis";
          ...
          <div className="d-flex align-items-center gap-2">
            <div
              style={{
                width: 4,
                height: 20,
                background: "linear-gradient(to bottom, #4338ca, #1e3a5f)",
                borderRadius: 2,
              }}
            />
          ...
          {!activeConv ? (
            <div className="d-flex flex-column align-items-center justify-content-center flex-grow-1 py-5">
              ...
              <p className="text-white-50 small mb-3">
                Seleccioná un análisis rápido o iniciá un chat
              </p>
              <button
                className="btn btn-sm d-flex align-items-center gap-1"
                onClick={startNewChat}
            ...
            <>
              ...
              <div
                ...
                style={{
                  ...
                  overflow: "auto",
                  minHeight: 200,
                  maxHeight: activeConv ? "calc(100vh - 340px)" : 200,
                }}
              >
                {/* Chat title */}
                <span className="small fw-medium text-white-50">
                  {activeConv.title}
                </span>

                {activeConv.messages.length === 0 ? (
                  <p className="text-white-50 small text-center py-4">
                    ...
                    >
                      <div className="d-flex gap-1">
                        <div
                          ...
                          style={{ animationDelay: "0s", color: "#818cf8" }}
                        />
                        <div
                          className="spinner-grow spinner-grow-sm"
                          style={{ animationDelay: "0.15s", color: "#818cf8" }}
                        />
                        <div
                          className="spinner-grow spinner-grow-sm"
                          style={{ animationDelay: "0.3s", color: "#818cf8" }}
                        />
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="d-flex gap-2 align-items-center">
                  <input
                    type="text"
                    className="form-control border-0 flex-fill text-white"
                    ...
                    style={{
                      fontSize: "0.875rem",
                      boxShadow: "none",
                      background: "rgba(255,255,255,0.06)", // fondo sutil oscuro
                      color: "#fff",
                    }}
                  />
                  <button
